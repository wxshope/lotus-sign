...
func InitRepo(path string) error {
	kstorePath := filepath.Join(path, fsKeystore)
...
func NewWallet(path string) (*LocalWallet, error) {
	w := &LocalWallet{
		keys:     make(map[address.Address]*key.Key),
		keystore: filepath.Join(path, fsKeystore),
	}
...
func (w *LocalWallet) tryFind(addr address.Address) (types.KeyInfo, error) {
	...
	ki, err := w.local.Get(KNamePrefix + addr.String())
	if err == nil {
		return ki, err
	}
	...
	ki, err = w.local.Get(KNamePrefix + tAddress)
	if err != nil {
		return types.KeyInfo{}, err
	}
	...
	err = w.local.Put(KNamePrefix+addr.String(), ki)
	if err != nil {
		return types.KeyInfo{}, err
	}
...
func (w *LocalWallet) WalletImport(ctx context.Context, ki *types.KeyInfo) (address.Address, error) {
	...
	if err := w.local.Put(KNamePrefix+k.Address.String(), k.KeyInfo); err != nil {
...
func (w *LocalWallet) WalletNew(ctx context.Context, typ types.KeyType) (address.Address, error) {
	...
	_, err = w.local.Get(KDefault)
	if err != nil {
		if !xerrors.Is(err, types.ErrKeyInfoNotFound) {
			return address.Undef, err
		}

		if err := w.local.Put(KDefault, k.KeyInfo); err != nil {
...
func (w *LocalWallet) walletDelete(ctx context.Context, addr address.Address) error {
	...
	if err := w.local.Delete(KTrashPrefix + k.Address.String()); err != nil && !xerrors.Is(err, types.ErrKeyInfoNotFound) {
		return xerrors.Errorf("failed to purge trashed key %s: %w", addr, err)
	}

	if err := w.local.Delete(KNamePrefix + k.Address.String()); err != nil {
		return xerrors.Errorf("failed to delete key %s: %w", addr, err)
	}
	...
	_ = w.local.Delete(KNamePrefix + tAddr)
...
func (w *LocalWallet) deleteDefault() {
	w.lk.Lock()
	defer w.lk.Unlock()
	if err := w.local.Delete(KDefault); err != nil {
		if !xerrors.Is(err, types.ErrKeyInfoNotFound) {
...
func (w *LocalWallet) SetDefault(a address.Address) error {
	...
	ki, err := w.local.Get(KNamePrefix + a.String())
	if err != nil {
		return err
	}
	...
	if err := w.local.Put(KDefault, ki); err != nil {
		return err
	}

	return nil
}
func (a *LocalWallet) WalletVerify(ctx context.Context, k address.Address, msg []byte, sig *crypto.Signature) (bool, error) {
	return sigs.Verify(sig, k, msg) == nil, nil
}

func (a *LocalWallet) WalletDefaultAddress(ctx context.Context) (address.Address, error) {
	return a.GetDefault()
}
